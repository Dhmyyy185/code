...
//#define EXPECT_EQ_STRING(expect, actual, len) EXPECT_EQ_BASE( memcmp(expect, actual, len) == 0 && strlen(expect) == len, expect, actual, "%s")
...
void fun() {
    //打开JSON文件
    FILE* fp;
    errno_t err = fopen_s(&fp, "C:\\Users\\Myy\\source\\repos\\leptjson_test\\War3FixFont.json", "r");
    if (fp == NULL || err != 0) {
    ...
    char* s;
    lept_init(&v1);
    //lept_parse(&v1, "{\"UseHotKey\": true,\"HotKey\" : \"0,1,1,D\",\"WindowMode\" : 1,\"UseTimer\" : false,\"TimerInterval\" : 60,\"FixThreshold\" : 1}");
    lept_parse(&v1, json_str);
    s = lept_stringify(&v1, NULL);
    printf("%s\n", s);
    //释放内存
    //printf("%d\n", v1.type);
    free(json_str);
